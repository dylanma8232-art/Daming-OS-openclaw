...
CAPABILITIES: tuple[Capability, ...] = (
    ...
    Capability("bitable_sync", ("bitable_sync.py", "cleanup_bitable_no_id.py"), "daily@03:20"),
    ...
    Capability("feishu_faq", ("feishu-group-faq.py",), "daily"),
    Capability("path_rules", ("path_scoped_rules.py",), "retrieval/deploy"),
    Capability("golden_paths", ("golden_path.py",), "daily"),
)
...
def missing_runtime_triggers(runtime: object) -> List[str]:
    ...
    hooked = {"hot_capture", "hybrid_recall", "graph_activation", "semantic_cache", "context_compaction",
              ...
              "path_rules"}
    scheduled = {
        "durable_storage": "deep-sleep-agent", "consolidation": "memory-consolidator",
        "wiki_graph_sync": "wiki-sync", "bitable_sync": "bitable-sync", "reviews_quality": "daily-review",
        "archive_cleanup": "session-cleanup", "skill_lifecycle": "workflow-distillation",
        "growth_extraction": "growth-event-clustering", "twin_audit": "growth-audit",
        "proactive_health": "growth-health", "golden_paths": "agent-as-judge", "feishu_faq": "feishu-faq-extraction",
    }
    return [capability.name for capability in CAPABILITIES
            if capability.name not in hooked and (capability.name not in scheduled or scheduled[capability.name] not in jobs)]
